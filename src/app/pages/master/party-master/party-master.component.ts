...
export class PartyMasterComponent implements OnInit {
  ...
  ngOnInit(): void {
  this.getPartyList()
      // this.http.get('assets/json/userdata.json').subscribe({
      // next: (response) => {
...
export class partyMasterDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.action === 'Edit') {
      ...
      // this.partyForm.controls['disabled'].setValue(this.local_data.disabled)
  ...
  doAction(): void {
    const payload = {
      ...
      // disabled: this.partyForm.value.disabled,
